...
GraphicalLiquidCrystalPCD8544::GraphicalLiquidCrystalPCD8544(unsigned char mosiPin, unsigned char clockPin, unsigned char rstPin, unsigned char dcPin, unsigned char scePin)
        : GraphicalLiquidCrystal(0x54, 0x30), mosiPin(mosiPin), clockPin(clockPin), rstPin(rstPin), dcPin(dcPin), scePin(scePin), currentScroll(0x00) {
...
void GraphicalLiquidCrystalPCD8544::flush() {
...
bool GraphicalLiquidCrystalPCD8544::streak(unsigned char x, unsigned char line, unsigned char streak) {
    ...
    // Should save into internal buffer? NO. Who called should do that.
    send(streak);
    return true;
}
...
void GraphicalLiquidCrystalPCD8544::scroll(ScrollDirection direction, unsigned char lines) {
    ...
    if (direction == SCROLL_UP) {
        ...
        if (lines <= currentScroll) {
            ...
            line = GRAPHICAL_LIQUID_CRYSTAL_PCD8544_HEIGHT_PAGES - (lines - currentScroll);
        }
    }
    scrollTo(line);
}

void inline GraphicalLiquidCrystalPCD8544::scrollUp(unsigned char lines) {
    scroll(SCROLL_UP, lines);
}

void inline GraphicalLiquidCrystalPCD8544::scrollDown(unsigned char lines) {
